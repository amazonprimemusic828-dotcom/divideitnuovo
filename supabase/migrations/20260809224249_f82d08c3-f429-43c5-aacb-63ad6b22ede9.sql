...
CREATE OR REPLACE FUNCTION public.get_public_groups()
 RETURNS TABLE(id uuid, service_name text, service_type text, plan_type text, total_cost numeric, currency text, max_members integer, billing_date integer, description text, status text, is_public boolean, created_date timestamp with time zone, closed_at timestamp with time zone, owner_id uuid, member_count integer, cover_image_url text, admin_name text)
 LANGUAGE sql
...
AS $function$
  SELECT g.id, g.service_name, g.service_type, g.plan_type,
         ...
         g.cover_image_url,
         (SELECT NULLIF(btrim(m2.user_name), '') FROM public.memberships m2
           WHERE m2.group_id = g.id AND m2.role = 'admin'
           ORDER BY m2.created_date ASC LIMIT 1)
  FROM public.groups g
  WHERE g.is_public = true AND g.status = 'active' AND g.closed_at IS NULL
  ORDER BY g.created_date DESC;
$function$;

DROP FUNCTION IF EXISTS public.get_public_group(uuid);
